...
class UseMacroNode(template.Node):
    def __init__(self, getMacro):
        self.getMacro = getMacro
    def render(self, context):
        macro, filter_expressions = self.getMacro(context)
        nodelist = macro.nodelist
        args = macro.args
        
        for (arg, fe) in [(args[i], filter_expressions[i]) for i in range(len(args))]:
