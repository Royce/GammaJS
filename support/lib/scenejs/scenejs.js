...
var SceneJS_memoryModule = new (function() {
    ...
    function outOfMemory(description) {
        ...
        throw SceneJS_errorModule.fatalError(new SceneJS.OutOfVRAMException(
...
var SceneJS_processModule = new (function() {
    ...
    this.createProcess = function(cfg) {
        if (!activeSceneId) {
            throw SceneJS_errorModule.fatalError(new SceneJS.NoSceneActiveException("No scene active - can't create process"));
        }
        var group = groups[activeSceneId];
        var i = 0;
        while (true) {
...
var SceneJS_sceneModule = new (function() {
    ...
    function findCanvas(canvasId) {
        ...
        if (!canvasId) {
            ...
            if (!canvas) {
                throw SceneJS_errorModule.fatalError(new SceneJS.CanvasNotFoundException
            ...
            canvas = document.getElementById(canvasId);
            if (canvas) {
                SceneJS_loggingModule.info("SceneJS.Scene binding to canvas '" + canvasId + "'");
            } else {
        ...
        if (!context) {
            throw SceneJS_errorModule.fatalError(new SceneJS.WebGLNotSupportedException
    ...
    this.deactivateScene = function() {
        if (!activeSceneId) {
            throw SceneJS_errorModule.fatalError("Internal error: no scene active");
        }
